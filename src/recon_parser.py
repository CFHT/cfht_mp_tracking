...
def main():
    ...
    parser.add_argument('--iteration', type=int, default=0, help="Which iteration through the list is this?")
    parser.add_argument('--object', help='Select this target regardless', default=None)
    args = parser.parse_args()
    url = eventListUrl[args.list]

    if args.object is None:
        print("Working on events in list: {}".format(url))
        for target in parse_recon_table(url, iter=args.iteration):
            minor_planet_ephemeris.build_ephem_files(target, args.start_time, 
                                                     args.stop_time, 
                                                     ephem_format="CFHT API", 
                                                     runid='17BC08')
    else:
        minor_planet_ephemeris.build_ephem_files(args.object, args.start_time, 
                                                 args.stop_time, 
                                                 ephem_format="CFHT API", 
                                                 runid='17BC08')
...
def parse_recon_table(url, iter=0):
    ...
    if 'ET' in table.colnames:
        ...
        cond = numpy.all((event_time > Time('2018-09-01 00:00:00', scale='utc'),
                          event_time < Time('2019-01-30 00:00:00', scale='utc'),
                          cond), axis=0)
    else:
